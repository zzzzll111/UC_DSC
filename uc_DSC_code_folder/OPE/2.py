...
if __name__ == "__main__":
    ...
    sheet.append(["Iteration", "Worker Time (seconds)", "Requester Time (seconds)"])
    ...
    for i in range(iterations):
        ...
        worker_execution_time = end_worker_time - start_worker_time

        # 计算 requester1 部分的执行时间
        start_requester_time = time.time()
        requester1.Location_Policy_Generation(level)
        requester1.Location_Proof_Verification(worker1, ciphx, ciphy, x_min_ope, x_max_ope, y_min_ope, y_max_ope)
        end_requester_time = time.time()

        requester_execution_time = end_requester_time - start_requester_time

        # 记录每次迭代的时间到Excel中
        sheet.append([i + 1, worker_execution_time, requester_execution_time])
